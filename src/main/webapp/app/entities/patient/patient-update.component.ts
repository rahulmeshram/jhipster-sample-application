...
import { GeneralMstService } from 'app/entities/general-mst';
...
export class PatientUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.generalMstService
      .query()
      .pipe(
        filter((mayBeOk: HttpResponse<IGeneralMst[]>) => mayBeOk.ok),
        map((response: HttpResponse<IGeneralMst[]>) => response.body)
      )
      .subscribe((res: IGeneralMst[]) => (this.generalmsts = res), (res: HttpErrorResponse) => this.onError(res.message));
  }
  ...
  trackGeneralMstById(index: number, item: IGeneralMst) {
    return item.id;
  }
}
